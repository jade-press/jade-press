...
exports.init = async function (initOptions) {
	let userId = cid()
	await exports.initMeta()
	await exports.initOptions()
	let defaultGroup = await exports.initGroup()
	let defaultCat = await exports.initCat()
	let resetPassword = await exports.initResetPassword(userId)
	let opts = {
		group: defaultGroup
	...
	let user = await exports.initUser(opts, userId)
	await exports.initPost(defaultCat, user)

	return Promise.resolve()

}

exports.initOptions = async function () {
	return Promise.resolve()
}

exports.initMeta = async function () {
	await db.collection('meta').insertOne({
		name: 'jade press version'
		,value: pack.version
...
exports.initResetPassword = async function(userId) {
	let dt = new Date()
	...
	await db.collection('reset_password').insertOne(rs)
	return Promise.resolve(rs._id)
}

exports.initUser = async function (option, _id) {
...
exports.initCat = async function () {

	let did = await newId()
	let did1 = await newId()
	let _id = cid()
	let _id1 = cid()
	let date = new Date()
	let date1 = new Date()
	...
	await db.collection('cat').insertOne(root)
	await db.collection('cat').insertOne(defaultCat)
	return Promise.resolve(defaultCat)

}

exports.initGroup = async function () {
	...
	await db.collection('group').insertMany(arr)
...
`
	...
	let html = await jadeParser(content, {})
	let style = ''
	let css = ''
	let script = 'console.log("init")'
	let cats = [cat]
	let visit = 0
	let createBy = user
	let meta = {}
