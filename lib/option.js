...
exports._add = async function (_body) {
	...
	let ind = await db.collection('option').findOne({
		_id: body._id
	})
	...
	if(!validateResult.errCount) {
		...
		let res = await db.collection('option').insertOne(body)
...
exports._del = async function (_body) {
	...
	res = await db.collection('option').deleteOne(opt)
...
exports.get = async (ctx, next) => {
	...
	try {
		...
		let res = await db.collection('option').find(opt)
		.toArray()
...
exports.update = async (ctx, next) => {
	...
	try {
		...
		if(!validateResult.errCount) {
			...
			delete res._id
			var insd = await db.collection('option').updateOne({
				_id: _id
			}, { $set: res })
...
exports.getSystemInfo = async (ctx, next) => {
